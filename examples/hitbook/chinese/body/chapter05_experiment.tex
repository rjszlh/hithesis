...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{figures/chapter04/卡尔曼融合定位结果.png}
\caption{卡尔曼融合定位效果图}
\label{卡尔曼融合定位效果图}
\end{figure}

室内外定位切换效果如图\ref{室内室外切换效果图}所示。红色轨迹为GPS原始轨迹，黄色轨迹为融合结果。图\subref{室内室外切换效果图:outdoor}所示为车辆在室外运行时的定位结果，此时GPS虽存在一定波动，但融合轨迹整体保持平稳；图\subref{室内室外切换效果图:indoor}所示为车辆进入室内后的定位结果，在GPS信号丢失后，系统仍可依靠IMU和轮速计维持连续定位，轨迹依然平稳。实验过程中，融合定位结果以$120\,\mathrm{Hz}$频率持续输出，能够满足自动行驶对定位实时性的要求。

\begin{figure}[h]
\centering
\subfigure[室内环境\label{室内室外切换效果图:indoor}]{\includegraphics[width=0.4\textwidth]{figures/chapter04/室内环境切换结果.png}}
% \hfill
\subfigure[室外环境\label{室内室外切换效果图:outdoor}]{\includegraphics[width=0.4\textwidth]{figures/chapter04/室外环境切换结果.png}}
\caption{室内室外切换效果图}
\label{室内室外切换效果图}
\end{figure}
...
本章对两轮平衡车的运动性能、视觉感知能力及自主行驶能力进行了综合实验验证。基本行驶测试表明，车辆启动平稳、加减速响应迅速、转向可控，能够稳定完成越障与复杂路径行驶，验证了底盘结构与平衡控制系统的可靠性。嵌入式部署实验表明，感知系统具备较好的实时性与测距稳定性，能够满足低速移动场景下的障碍物检测需求。自主行驶能力测试表明，定位模块能够在GPS波动及室内外切换场景下保持较平稳、连续的轨迹输出；在动态干扰条件下，系统能够完成目标检测、路径重规划与平滑绕行，保持安全距离，运行过程整体稳定。
