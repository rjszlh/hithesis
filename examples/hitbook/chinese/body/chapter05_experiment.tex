...
为进一步分析避障过程中的轨迹、距离和速度变化，结合图像序列提取车辆运动轨迹，并在同一时间轴下计算车辆与障碍物的相对距离、速度大小和速度偏向角，结果如图\ref{动态避障数据分析}所示。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\textwidth]{figures/chapter05/动态避障反推分析四联图.pdf}
  \caption{动态避障过程数据分析}
  \label{动态避障数据分析}
\end{figure}

由图\ref{动态避障数据分析}可知，车辆沿全长约$16.05\,\mathrm{m}$的路径由右向左行驶，动态障碍物位于距起点约$5.00\,\mathrm{m}$处。车辆接近障碍物后产生局部绕行动作，最大横向偏移发生在$t=5.71\,\mathrm{s}$附近，轨迹相对直行基准线的偏移量约为$0.22\,\mathrm{m}$。在距离约束方面，车辆与障碍物的最小相对距离为$1.25\,\mathrm{m}$，高于$1.00\,\mathrm{m}$的安全阈值，说明车辆在避障过程中保留了一定安全裕度。速度变化与距离变化具有对应关系，当车辆进入障碍物附近时速度由正常行驶状态降低，最低速度约为$0.58\,\mathrm{m/s}$，随后随距离增大逐渐恢复；速度偏向角在$t=5.30\,\mathrm{s}$和$t=6.11\,\mathrm{s}$附近分别出现约$-19.5^\circ$和$18.5^\circ$的转向峰值，反映了车辆先偏转避让、再回正行驶的运动过程。

实验结果表明，在设定行驶速度范围内，规划控制环节能够维持$20\,\mathrm{Hz}$的运行频率；当行人进入原规划路径后，系统能够识别前方动态障碍并触发局部路径调整，车辆在安全范围内表现出减速和绕行动作，未观察到明显失稳现象。
