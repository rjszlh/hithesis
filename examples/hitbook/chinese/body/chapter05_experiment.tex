...
为验证前文所提出两轮平衡车总体方案及关键技术的有效性，本章从整车运动性能、双目视觉环境感知能力以及自主避障功能三个方面开展实验研究。首先，通过基本行驶性能测试分析车辆在启动、加减速、转向及越障等典型工况下的姿态稳定性与操控性能；其次，结合嵌入式平台部署与外场测距实验，对双目视觉感知算法的实时性与测距精度进行评估；最后，在动态障碍物干扰场景下，对整车“感知—规划—控制”各模块的协同工作效果与系统运行稳定性进行验证，从而为所构建系统的工程可行性与应用可靠性提供实验依据。
...
在完成两轮平衡车行驶性能与环境感知能力的独立验证后，进一步开展整车避障实验，验证“感知—规划—控制”完整链路的协同工作效果与系统稳定性。该实验重点评估在动态干扰条件下，车辆对突发障碍物的响应能力与安全裕度。
...
本章对两轮平衡车的运动性能、视觉感知能力及自主避障效果进行了综合实验验证。基本行驶测试表明，车辆启动平稳、加减速响应迅速、转向可控，能够稳定完成越障与复杂路径行驶，验证了底盘结构与平衡控制系统的可靠性。嵌入式部署实验显示，经过TensorRT（FP16）优化后，感知系统在640$\times$480分辨率下实现约30 Hz端到端刷新频率，满足低速运行需求。2--10 m范围内整体平均相对误差为3.52\%，误差随距离增加呈上升趋势，符合立体视觉测距规律。避障实验表明，系统能够在动态干扰条件下完成目标检测、路径重规划与平滑绕行，保持安全距离，运行过程整体稳定。综上，实验结果验证了所构建系统在工程应用中的可行性与稳定性。
