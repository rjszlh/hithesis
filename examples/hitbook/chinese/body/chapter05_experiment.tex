...
真实距离采用手持式激光测距仪进行测量，其标称精度为$\pm 3$ mm。测量时以相机光心为起点，选取目标可见表面的几何中心作为测量点，以保证与视觉测距输出的空间定义保持一致。对于每一类障碍物在每一个距离处的测距结果，均在目标对象及场景布置保持不变的条件下重复测量5次，并取平均值作为该距离下的视觉测距结果；激光测距仪测得距离作为参考真值，用于评估双目测距结果的误差。
...
由表\ref{不同类别障碍物在各距离下的测距结果与相对误差}和图\ref{不同类别障碍物相对误差随距离变化}可知，在全部30组测量数据中，整体平均相对误差约为3.56\%。不同距离下的平均相对误差分别为：$2\,\mathrm{m}$约1.42\%、$4\,\mathrm{m}$约2.53\%、$6\,\mathrm{m}$约2.72\%、$8\,\mathrm{m}$约4.57\%、$10\,\mathrm{m}$约6.55\%。按照前文提出的分阶段平均误差指标进行评价，系统在$2\,\mathrm{m}$、$6\,\mathrm{m}$和$10\,\mathrm{m}$典型距离下的平均相对误差分别低于2\%、5\%和10\%，满足近距离、中距离与远距离测距精度要求。
