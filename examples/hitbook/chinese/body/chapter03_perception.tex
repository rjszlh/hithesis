...
在相机参数固定的条件下，视差与深度呈反比关系。理想双目成像模型如图\ref{理想双目模型}所示，在该模型下可建立像素坐标与三维空间坐标之间的几何映射关系。
...
双目视觉测距基于针孔成像几何模型。为便于分析，假设左右相机光轴平行、成像平面共面，双目基线沿水平方向布置，且图像已完成极线校正。在该理想条件下，空间点$P$在左右相机像平面上的像素坐标分别记为$P_l(x_l,y_l)$和$P_r(x_r,y_r)$，对应点满足$y_l=y_r$，视差仅沿水平方向产生。根据相似三角形关系，可得到空间点深度与视差之间的关系，如式~\eqref{eq:ch3-geometry}所示：
\begin{equation}\label{eq:ch3-geometry}
Z=\frac{fB}{d}
\end{equation}
\begin{tabularx}{\textwidth}{@{}l@{\quad}r@{———}X@{}}
式中& $P$ & 空间点在相机坐标系下的三维点；\\
& $P_l(x_l,y_l),\,P_r(x_r,y_r)$ & 空间点在左右相机像平面上的像素坐标；\\
& $Z$ & 空间点到相机的深度值；\\
& $d$ & 左右图像对应点的水平视差；\\
& $f$ & 相机有效焦距；\\
& $B$ & 双目基线长度；\\
...
实际双目系统中普遍存在镜头畸变与安装误差，若不进行标定将导致测距结果偏离理想几何模型。为此，本文采用张正友标定方法，利用MATLAB的Stereo Camera Calibrator采集多组棋盘格图像进行双目标定。标定后可获得左右相机的内参矩阵$\mathbf{K}_l,\mathbf{K}_r$、畸变参数$\mathbf{D}_l,\mathbf{D}_r$以及双目系统的相对位姿$\mathbf{R},\mathbf{t}$，标定结果如图\ref{双目相机标定结果}所示。
...
相机内参矩阵，其形式如式~\eqref{eq:ch3-intrinsic}所示，其用于建立相机坐标系与像素坐标系之间的映射关系，并为图像去畸变和立体校正提供投影参数。
\begin{equation}\label{eq:ch3-intrinsic}
\mathbf{K}=
\begin{bmatrix}
f_x & 0 & c_x \\
0 & f_y & c_y \\
...
\begin{tabularx}{\textwidth}{@{}l@{\quad}r@{———}X@{}}
式中& $\mathbf{K}$ & 相机内参矩阵；\\
& $f_x,f_y$ & 分别为$x$、$y$方向的等效焦距；\\
& $c_x,c_y$ & 主点坐标。\\
\end{tabularx}\vspace{3.15bp}

内参矩阵用于建立相机坐标系与像素坐标系之间的映射关系，并为图像去畸变和立体校正提供投影参数，其在校正过程中的作用可表示为式~\eqref{eq:ch3-rectify}：
\begin{equation}\label{eq:ch3-rectify}
\tilde{\mathbf{p}}' \sim \mathbf{H}\tilde{\mathbf{p}},\quad \mathbf{H}=\mathbf{K}'\mathbf{R}_{\mathrm{rect}}\mathbf{K}^{-1}
\end{equation}
\begin{tabularx}{\textwidth}{@{}l@{\quad}r@{———}X@{}}
式中& $\tilde{\mathbf{p}}$ & 原始图像中的像素齐次坐标；\\
& $\tilde{\mathbf{p}}'$ & 校正后的像素齐次坐标；\\
& $\mathbf{H}$ & 图像校正单应矩阵；\\
& $\mathbf{K}'$ & 校正后相机内参矩阵；\\
& $\mathbf{R}_{\mathrm{rect}}$ & 立体校正旋转矩阵。\\
\end{tabularx}\vspace{3.15bp}

其中，$\mathbf{K}$和$\mathbf{K}'$分别参与原始图像与校正图像的投影变换，$\mathbf{R}_{\mathrm{rect}}$由双目标定得到的相对位姿计算获得。在此基础上，首先根据内参矩阵和畸变参数对原始图像进行去畸变处理，以消除镜头畸变对成像位置的影响。设畸变像素点为$\tilde{\mathbf{p}}_d$，无畸变像素点为$\tilde{\mathbf{p}}_u$，则去畸变过程可表示为式~\eqref{eq:ch3-undistort}：
\begin{equation}\label{eq:ch3-undistort}
\tilde{\mathbf{p}}_u=\mathcal{D}^{-1}(\tilde{\mathbf{p}}_d)
\end{equation}
\begin{tabularx}{\textwidth}{@{}l@{\quad}r@{———}X@{}}
式中& $\tilde{\mathbf{p}}_d,\tilde{\mathbf{p}}_u$ & 去畸变前后的像素齐次坐标；\\
& $\mathcal{D}^{-1}(\cdot)$ & 由标定获得的畸变参数确定的去畸变映射。\\
\end{tabularx}\vspace{3.15bp}

随后，根据双目系统的相对位姿对左右图像进行立体校正，使两相机成像平面尽可能共面，并使对应点落在同一水平极线上。对应点仍满足极几何约束，如式~\eqref{eq:ch3-epipolar}所示：
\begin{equation}\label{eq:ch3-epipolar}
\tilde{\mathbf{p}}_r^{\mathrm{T}}\mathbf{F}\tilde{\mathbf{p}}_l=0
\end{equation}
\begin{tabularx}{\textwidth}{@{}l@{\quad}r@{———}X@{}}
式中& $\mathbf{F}$ & 基础矩阵；\\
& $\tilde{\mathbf{p}}_l,\tilde{\mathbf{p}}_r$ & 左右图像中对应点的齐次像素坐标。\\
\end{tabularx}\vspace{3.15bp}

经过去畸变与立体校正后，左右图像中同一空间点的投影将尽可能落在同一水平扫描线上，即满足$y_l' \approx y_r'$。由此可将对应点匹配由二维搜索简化为沿水平方向的一维搜索，从而提高视差估计的稳定性。

基于标定结果，对左右图像进行去畸变与立体校正处理后，其极线能够较好地对齐至水平方向，校正效果如图\ref{图像校正处理结果}所示。校正后的图像可稳定用于视差估计与深度计算，为后续障碍物检测与测距算法提供可靠的几何基础。
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.99\textwidth]{figures/chapter03/SceneFlow性能与FLOPs对比.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.99\textwidth]{figures/chapter03/KITTI2015性能与FLOPs对比.png}
  \caption{KITTI 2015数据集上的性能与FLOPs对比}
  \label{KITTI2015数据集上的性能与FLOPs对比}
\end{figure}
\FloatBarrier

在SceneFlow数据集上，HAGVNet在仅$20.08\,\mathrm{GFLOPs}$的条件下取得$0.73\,\mathrm{px}$的$EPE$，在计算量显著低于多数方法的情况下保持了具有竞争力的精度表现，其精度与复杂度关系如图\ref{SceneFlow数据集上的性能与FLOPs对比}所示；在KITTI 2012与KITTI 2015数据集上，HAGVNet的整体性能优于StereoNet\cite{khamisStereonetGuidedHierarchical2018a}、DeepPrunerFast\cite{duggalDeepprunerLearningEfficient2019a}、MobileStereoNet-2D\cite{shamsafarMobilestereonetLightweightDeep2022a}与AANet\cite{xuAANetAdaptiveAggregation2020}等方法。与HITNet\cite{tankovichHitnetHierarchicalIterative2021a}及Fast-ACVNet系列\cite{xuAccurateEfficientStereo2023}相比，HAGVNet在$D1$-all指标上仅高出约0.2\%--0.3\%，但其计算复杂度仅为$20.08\,\mathrm{GFLOPs}$，分别约为HITNet（$50.23\,\mathrm{GFLOPs}$）的40\%和Fast-ACVNet（$79.34\,\mathrm{GFLOPs}$）的25\%，在保持接近精度的同时显著降低了计算开销，其在KITTI 2015数据集上的性能与FLOPs关系如图\ref{KITTI2015数据集上的性能与FLOPs对比}所示。图\ref{HAGVNet与多种主流方法在KITTI数据集上的部分表现}给出了部分定性结果对比，可见在弱纹理区域与大面积平坦区域，HAGVNet能较好保持结构连续性并抑制局部匹配噪声。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.99\textwidth]{figures/chapter03/HAGVNet在KITTI2015部分结果对比.png}
