...
在该流程中，模型首先由骨干网络提取左右图像的多尺度特征，并分别利用原图$1/8$分辨率与$1/4$分辨率特征构建粗尺度代价体与细尺度代价体。粗尺度代价体经轻量聚合与视差回归后得到粗尺度视差先验$d_{\text{coarse}}$。随后，将$d_{\text{coarse}}$上采样至细尺度，并据此生成平滑的高斯注意力先验。高斯注意力形式见式~\eqref{eq:ch3-attention}：
...
设输入代价体为$X$。该模块首先通过深度卷积扩大感受野，以增强对局部上下文的建模能力，如式~\eqref{eq:ch3-conv1}所示：
...
在完成代价聚合后，对代价体采用soft-argmax操作预测最终视差图。具体而言，首先对每个像素位置处所有视差对应的代价值施加softmax归一化，得到视差概率分布，如式~\eqref{eq:ch3-prob}所示：
...
为验证上述轻量化立体匹配模型的有效性与泛化能力，本文采用“合成数据预训练+真实数据微调”的两阶段训练策略，并结合主流评价指标对模型性能进行定量分析。相关训练配置与评价标准不仅用于考察模型的匹配性能，也作为后文消融实验、对比实验及嵌入式部署分析的统一实验依据。
...
通过上述评价指标与分阶段训练策略，可系统分析模型在精度、鲁棒性及真实场景泛化能力方面的表现，并为后续消融实验和方法对比验证提供依据。
...
为验证所选YOLO11n目标检测模型在本文障碍物识别任务中的检测性能，并为后续目标空间定位与测距融合提供可靠的前端感知结果，本文对目标检测模型的训练过程及评价指标进行统一说明。模型训练与对比实验均在相同软硬件环境下完成，具体包括：Win10操作系统、NVIDIA GeForce RTX 4090显卡、Intel Core i5-9600KF处理器，深度学习框架采用PyTorch 1.9，编程语言为Python 3.8。

在目标检测任务中，常用评价指标包括精确率（Precision）、召回率（Recall）和平均精度均值（mAP）。其中，精确率表示预测为正样本的目标中真正为正样本的比例，如式~\eqref{eq:ch3-precision}所示：
...
在式~\eqref{eq:ch3-precision}和式~\eqref{eq:ch3-recall}中使用的符号说明如下。$mAP$表示各类别平均精度（Average Precision, AP）的均值，用于综合评价模型在多类别检测任务中的整体性能，如式~\eqref{eq:ch3-map}所示。
...
上述评价指标可从检测准确性、召回能力及整体识别性能三个方面对YOLO11n进行量化分析，并为后续目标空间定位提供前端感知质量依据。
