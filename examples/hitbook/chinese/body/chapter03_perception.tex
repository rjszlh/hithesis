...
环境感知一直以来是自动行驶的重点与难点。现有方法中，基于激光雷达的感知手段虽然精度较高，但其设备成本昂贵；而基于单目视觉的深度估计则存在精度不足和稳定性差的问题。另一方面，仅依赖基于目标识别的环境感知方法容易受到漏检和误检的影响，难以满足高可靠性的自动行驶感知需求。根据环境感知的需求，研究基于深度学习的立体匹配算法，在保证处理速度的前提下获取高质量的视差图，并通过双目相机参数计算得到深度图；训练基于深度学习的目标检测算法，并设计算法将深度图与目标检测结果结合，对障碍物目标的空间定位。最终实现对环境的有效感知。
