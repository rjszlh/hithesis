...
在YOLOv11系列的不同规模模型中，YOLO11n（nano版本）在精度与推理时间之间取得了较优折中。根据Ultralytics官方在COCO数据集上的检测模型指标\cite{jocherUltralyticsYOLO112024}，在输入尺寸为$640\times640$时，YOLO11n的mAP@0.5:0.95为$39.5\%$，参数量为$2.6\,\mathrm{M}$，计算量为$6.5\,\mathrm{B}$ FLOPs；其在CPU ONNX平台上的推理延迟为$56.1\pm0.8\,\mathrm{ms}$，在NVIDIA T4 TensorRT10平台上的推理延迟为$1.5\pm0.0\,\mathrm{ms}$。因此，YOLO11n能够在保持较低模型规模和毫秒级GPU推理延迟的同时提供可接受的检测精度，适合嵌入式视觉系统中作为实时目标检测前端。综合工程可部署性与系统整体算力预算，最终选定YOLO11n作为本系统的目标检测模型。
...
在式~\eqref{eq:ch3-precision}和式~\eqref{eq:ch3-recall}中使用的符号说明如下。mAP表示各类别平均精度（Average Precision, AP）的均值，用于综合评价模型在多类别检测任务中的整体性能，如式~\eqref{eq:ch3-map}所示。
\begin{equation}\label{eq:ch3-map}
\mathrm{mAP}=\frac{1}{N}\sum_{c=1}^{N} \mathrm{AP}_c
\end{equation}
\begin{tabularx}{\textwidth}{@{}l@{\quad}r@{———}X@{}}
式中& $TP$ & 将正样本预测为正样本的数量；\\
& $FP$ & 将负样本预测为正样本的数量；\\
& $FN$ & 将正样本预测为负样本的数量；\\
& $N$ & 类别总数；\\
& $\mathrm{AP}_c$ & 第$c$类在Precision-Recall曲线下的面积。\\
\end{tabularx}\vspace{3.15bp}
