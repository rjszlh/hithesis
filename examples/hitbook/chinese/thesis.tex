...
%     ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\documentclass[fontset=windows,type=master,campus=harbin]{hithesisbook}
% 此处选项中不要有空格
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 必填选项
...
% 页边注区域宽度：调大后 marginnote 更容易正常换行
\setlength{\marginparwidth}{2.5cm}
% 正文和页边注之间的间距
\setlength{\marginparsep}{0.2cm}
% 最终输出前记得关闭页边注；取消下一行注释后，所有 \marginnote 都会被隐藏。
...
% \cmidrule -> \cmidrulewidth
% \bottomrule -> \heavyrulewidth
\setlength{\heavyrulewidth}{1.5pt} % \toprule / \bottomrule
\setlength{\lightrulewidth}{0.75pt} % \midrule
\setlength{\cmidrulewidth}{0.5pt}  % \cmidrule

% 仅在真实表格中调整行高，避免影响公式中的 array 等环境
\newcommand{\thesistablearraystretch}{1.2}
\ExplSyntaxOn
\NewDocumentEnvironment{thesistabular}{ O{} m }
  {
    \begingroup
    \renewcommand{\arraystretch}{\thesistablearraystretch}
    \tl_if_blank:nTF {#1}
      { \begin{tabular}{#2} }
      { \begin{tabular}[#1]{#2} }
  }
...
\tableofcontents %目录
\mainmatter
\include{body/chapter01_intro}
\include{body/chapter02_design_control}
\include{body/chapter03_perception}
\include{body/chapter04_navigation}
\include{body/chapter05_experiment}
\backmatter
...
\bibliography{ref} % 参考文献（无引用时自动为空）
% \nocite{*} % 暂不处理正文引用，先保留参考文献占位
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%-- 注意：以下本硕博、博后书序不一致 --%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 本科书序（哈尔滨、深圳校区）
...
\authorization*[back/原创性声明和使用权限.pdf] % 使用扫描件，自动添加页码（扫描件中无页码时使用）
\include{back/acknowledgements} %致谢
